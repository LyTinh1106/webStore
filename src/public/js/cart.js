document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateCartUI() {
      ...
      if (cartData.length === 0) {
        ...
        cartQty.innerText = '0';
        summaryText.innerText = `0 Item(s) selected`;
        subtotal.innerText = `SUBTOTAL: $0.00`;
        return;
      } else {
        ...
        const productHTML = `
          <div class="product-widget">
            ...
            <div class="product-body">
              ...
              <h4 class="product-price"><span class="qty">${product.qty}x</span>$${product.price.toFixed(2)}</h4>
            </div>
      ...
      summaryText.innerText = `${totalQty} Item(s) selected`;
    ...
    // Xử lý add to cart từ trang danh sách sản phẩm
    document.querySelectorAll('.add-to-cart-btn').forEach(button => {
      button.addEventListener('click', function (e) {
        ...
        const priceText = productCard.querySelector('.product-price')?.innerText.trim().replace('$', '') || '0';
    ...
    // Xử lý add to cart từ trang chi tiết sản phẩm
